...
def Retrieve_Data():
    try:
        ...
        global Deliverables_queries, number_of_Deliverables, user_Deliverables
        query_5 = """INSERT INTO deliverables VALUES (( select (case when not EXISTS  (select * from deliverables D where D.project_id=(SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'db_project' AND  TABLE_NAME  = 'projects')) then 1 else (select count(*)+1 from deliverables D where D.project_id=(SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'db_project' AND  TABLE_NAME  = 'projects')) end)),(SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'db_project' AND  TABLE_NAME  = 'projects'),%s,%s,%s);"""
